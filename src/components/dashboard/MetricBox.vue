    ...
    <header class="flex items-center gap-2">
      ...
      >
        <span v-for="item in seriesTotals.items" :key="item.name" class="flex items-center gap-1.5">
          <span class="size-2 rounded-circle" :class="item.tint === 'leaf' ? 'bg-leaf-500' : 'bg-peach-600'" />
      ...
      >
        <span v-for="l in sample.lines" :key="l.name" class="flex items-center gap-1.5">
          <span class="size-2 rounded-circle" :class="l.tint === 'leaf' ? 'bg-leaf-500' : 'bg-peach-600'" /> {{ l.name }}
        </span>
      </div>
