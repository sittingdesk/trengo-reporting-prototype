...
import { spanClass } from '@/lib/widgetLayout'
 ...
 *
 *  `uid` first: keying a metric widget by `metricId` alone means two cards for the same
 *  metric would collide, which is a bug the moment anything can add one. Every widget
 *  built through `reportFromTemplate` or `addWidget` carries a uid; the fallback is for
 *  any widget literal that predates it. */
function widgetKey(widget: Widget, i: number) {
  if (widget.uid) return widget.uid
...
// Per-widget widths live in `@/lib/widgetLayout` — shared with the widget library, which
// has to state a widget's size before you add it and decide whether it starts a row.
